...
def create_classifier(
    ...
    rcca_templates = get_rcca_model(cmeta, V)
    rcca_templates.fit(X, y)
    ...
    if n_keys != 0 and n_keys < V.shape[0]:
        Ts = rcca_templates.Ts_[:, 0, :]  # select component
    ...
    # Convert the hard-coded dict into an array of shape (n_neighbours, 2)
    neighbour_set = []
    for key, neigh_list in keyboard_dict.items():
        for neighbour in neigh_list:
            neighbour_set.append([key, neighbour])

    Ts = rcca_templates.Ts_[subset, 0, :]

    # Only optimise layout if a valid neighbour graph exists
    if len(neighbour_set) > 0:
        neighbours = np.asarray(neighbour_set, dtype = int).reshape(-1, 2)
        ...
        layout = np.arange(Ts.shape[0], dtype = int)
        logger.debug(
            f"Skipped optimal layout for {n_keys} keys because no neighbour graph is defined"
        )
