...
class Speller(object):
    ...
    def __init__(
        ...
        self.keyboard_output_cfg = self.cfg.get("speller", {}).get("keyboard_output", {})
        self._kbd = None

        if self.keyboard_output_cfg.get("enabled", False):
            if PynputController is None:
                logger.warn("keyboard_output.enabled=true but pynput could not be imported.")
            else:
                self._kbd = PynputController()
                logger.info("Keyboard output enabled (pynput). Keystrokes go to the currently focused window.")
        ...
        # Make the whole window semi-transparent (0.0..1.0)
        win_opacity = float(self.cfg["speller"]["screen"].get("window_opacity", 1.0))
        try:
            self.window.winHandle.set_opacity(win_opacity)
        except Exception as e:
            logger.warn(f"Could not set window opacity (backend/platform limitation): {e}")
    ...
    def handle_decoding_event(self) -> None:
        ...
        # Type into the currently focused app (outside PsychoPy) if enabled
        self._emit_to_focused_app(symbol = symbol, text_before = text_before, text_after = text)
    ...
    def _emit_to_focused_app(self, symbol: str, text_before: str, text_after: str) -> None:
        """
        Types into the currently focused OS window using pynput.
        Requires: [speller.keyboard_output].enabled = true
        """
        logger.info("SYMBOL ================================", symbol, text_before, text_after)
        if self._kbd is None:
            return

        try:
            kb = self._kbd
            cfg = self.keyboard_output_cfg

            # Ignore non-typing control keys
            if symbol in (self.cfg["speller"]["key_shift"], self.cfg["speller"]["key_text2speech"]):
                return

            # Space key (your logic uses the literal string "space")
            if symbol == self.cfg["speller"]["key_space"]:
                kb.press(PynputKey.space)
                kb.release(PynputKey.space)
                return

            # Backspace key (your logic turns "backspace" into "<-" via KEY_MAPPING)
            if symbol == self.cfg["speller"]["key_backspace"]:
                kb.press(PynputKey.backspace)
                kb.release(PynputKey.backspace)
                return

            # Clear key
            if symbol == self.cfg["speller"]["key_clear"]:
                if cfg.get("ctrl_a_clears", True):
                    # Select-all then delete (works in most apps)
                    with kb.pressed(PynputKey.ctrl):
                        kb.press("a")
                        kb.release("a")
                    kb.press(PynputKey.backspace)
                    kb.release(PynputKey.backspace)
                elif cfg.get("mirror_text_field", True):
                    # Backspace the known length
                    for _ in range(len(text_before)):
                        kb.press(PynputKey.backspace)
                        kb.release(PynputKey.backspace)
                return

            # Autocomplete key: update focused app to match the speller text field
            if self.cfg["speller"]["autocomplete"]["enabled"] and symbol == self.cfg["speller"]["key_autocomplete"]:
                if cfg.get("ctrl_a_clears", True):
                    with kb.pressed(PynputKey.ctrl):
                        kb.press("a")
                        kb.release("a")
                    ...
                    # Type only the suffix if we assume external text matches internal text
                    if cfg.get("mirror_text_field", True) and text_after.startswith(text_before):
                        kb.type(text_after[len(text_before):])
                    else:
                        kb.type(text_after)
                return

            # Default: type the literal symbol (letters/punctuation)
            kb.type(symbol)

        except Exception as e:
            logger.warn(f"Keyboard output failed: {e}")
