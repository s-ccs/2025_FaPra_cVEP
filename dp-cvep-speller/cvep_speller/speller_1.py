...
class Speller(object):
    ...
    def _safe_flip(self) -> bool:
        """Flip the PsychoPy window; return False if the GL context is gone."""
        if self.window is None:
            return False

        try:
            # pyglet backend has winHandle.context; when closing it can become None
            wh = getattr(self.window.backend, "winHandle", None)
            if wh is not None and hasattr(wh, "context") and wh.context is None:
                return False
            ...
            # Happens when the window is closing and pyglet already dropped the context
            return False
        except Exception:
            # Be conservative: if flip fails for any reason, stop drawing
            return False
