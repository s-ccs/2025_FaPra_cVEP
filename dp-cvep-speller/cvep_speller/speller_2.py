...
class Speller(object):
    ...
    def __init__(
        ...
        # Optional: emit decoded text as OS-level keyboard events to the currently focused window.
        # Enable via [speller.keyboard_output] in speller.toml
        self._kb_enabled = bool(self.cfg.get("speller", {}).get("keyboard_output", {}).get("enabled", False))
        self._kb_mirror_text_field = bool(self.cfg.get("speller", {}).get("keyboard_output", {}).get("mirror_text_field", True))
        self._kb_ctrl_select_all = bool(self.cfg.get("speller", {}).get("keyboard_output", {}).get("ctrl_a_clears", True))
        self._kb_controller = _KBController() if self._kb_enabled else None
...
def _kb_ctrl_key(self):
    """Return the platform-appropriate modifier for 'select all'."""
    # macOS uses Command, others typically use Control
    return _KBKey.cmd if sys.platform == "darwin" else _KBKey.ctrl

def _emit_text_delta(self, prev_text: str, new_text: str) -> None:
    """Mirror the speller's text field change into the currently focused OS window."""
    if not self._kb_enabled or self._kb_controller is None:
        return
    if not self._kb_mirror_text_field:
        return
    if prev_text == new_text:
        return

    # Case 1: append-only -> type suffix
    if new_text.startswith(prev_text):
        suffix = new_text[len(prev_text):]
        if suffix:
            self._kb_controller.type(suffix)
        return

    # Case 2: pure deletion (backspace) -> emit backspaces
    if prev_text.startswith(new_text):
        n_bs = len(prev_text) - len(new_text)
        for _ in range(n_bs):
            self._kb_controller.press(_KBKey.backspace)
            self._kb_controller.release(_KBKey.backspace)
        return

    # Case 3: mixed edit (e.g., autocomplete replace, clear) -> select all + delete, then type new_text
    if self._kb_ctrl_select_all:
        mod = self._kb_ctrl_key()
        try:
            # Select all
            with self._kb_controller.pressed(mod):
                self._kb_controller.press('a')
                self._kb_controller.release('a')
            # Delete
            self._kb_controller.press(_KBKey.backspace)
            self._kb_controller.release(_KBKey.backspace)
        except Exception as e:
            logger.warn(f"Keyboard select-all/delete failed: {e}")

    # Type full new text
    self._kb_controller.type(new_text)
    ...
    def handle_decoding_event(self) -> None:
        ...
        prev_text = text  # for optional OS keyboard mirroring
        ...
        self._emit_text_delta(prev_text, text)
